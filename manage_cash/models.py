...
class Category(models.Model):
    ...
    def can_delete(self):
        ...
        return not (self.subcategories.exists() or self.transactions.exists())
    ...
    class Meta:
        ...
        unique_together = (
            "name",
            "transaction_type",
        )  # Сохранение уникальности категории в готовом типе
...
class Subcategory(models.Model):
    ...
    def can_delete(self):
        """Можно ли безопасно удалить подкатегорию? - проверка в шаблоне на невозможность удаления"""
    ...
    def clean(self):
        ...
        if self.category_id:  # Проверяем ID
            if not self.type:
                raise ValidationError(
                    {"type": "Тип транзакции обязателен при выборе категории."}
                )
            if (
                self.category.transaction_type != self.type
            ):  # Только если category существует
                raise ValidationError(
                    {"category": "Категория не соответствует типу транзакции."}
                )
        ...
        if self.subcategory_id:
            if not self.category_id:
                raise ValidationError(
                    {"subcategory": "Нельзя выбрать подкатегорию без категории."}
                )
            if self.subcategory.category != self.category:
                raise ValidationError(
                    {"subcategory": "Подкатегория не соответствует категории."}
                )
